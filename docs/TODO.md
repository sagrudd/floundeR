...
Work from top to bottom unless a later slice is explicitly pulled
forward. Each slice should be small enough to commit independently.
Substantive work should include a semantic version bump and `NEWS.md`
entry.

## Slice 1: Package Baseline

Fix `DESCRIPTION` metadata so `R CMD check` passes initial validation on
the target R toolchain.

Reconcile `DESCRIPTION` dependencies with packages used in `R/`.

Confirm `aws.s3` remains an intentional dependency for ONT open-data
access, or replace it with a documented alternative before removing it.

Identify the relevant `../mnemosyne-docs` charters/contracts for
`floundeR`, `pod5-tools`, `bamana`, `grammateus`, synoptikon, and
Mnemosyne QC reporting before cross-repository implementation begins.

If no canonical `../mnemosyne-docs` entry exists for `bamana`, record
...
`../porkchop/README.md`, and relevant Sphinx docs as the governing
boundary for library-preparation QC integration until canonical docs are
added.

Decide and document whether generated `docs/` are committed during
normal development or only during releases.

Add or update CI for R package check.

Record the FAST5 retirement direction in `NEWS.md`.

Record and enforce the distribution boundary: floundeR, pod5-tools,
bamana, and porkchop are open-source; Grammateus remains private and
optional from the public package’s perspective.

Run `R CMD check --no-manual --no-build-vignettes .` and capture any
remaining failures as follow-up tasks.

Baseline check result on 2026-06-13: `R CMD check` now passes
DESCRIPTION and namespace validation, then stops because the local R
library lacks required imports/suggested packages. The FAST5 retirement
slice removed `rhdf5`; the remaining missing packages are CRAN and
Bioconductor dependencies needed by the current non-FAST5 package
surface.
...
Verify `rg -i "fast5|rhdf5"` only finds accepted migration/history
notes.
...
Add a Docker-backed development build and dependency audit path for the
current R package dependency surface.

Record current container dependency versions and support posture.

Ensure `testthat` is installed/declared with a current edition.

Add helper fixtures for sequencing summary, barcode summary, FASTQ,
FASTA, and minimal POD5 metadata tests.
...
Replace `eval(parse())` column specification construction with
structured
...
the source-package-safe Rust dependency for
...
Resolve or formally document the release-check compiled-code warning
from linking the Rust `std`/`pod5-tools` stack before
Bioconductor/CRAN-facing release work.
...
for anonymous S3 listings using `region = "eu-west-1"`.

Implement
[`ont_open_data_fetch()`](https://sagrudd.github.io/floundeR/reference/ont_open_data_fetch.md)
with explicit object-key selection, cache directory control, and no
implicit bulk downloads.

Add a named dataset helper for
`s3://ont-open-data/zymo_fecal_2025.05/raw/PAU85136/pod5/`.

Add named constants or helper accessors for the selected example
objects: `PAU85136_pass_279c9095_68316534_8289.pod5` and
`PAU85136_fail_279c9095_68316534_0.pod5`.

Record object key, size, last-modified timestamp, and local cache path
...
Add tests for valid POD5-like signatures, non-POD5 files, truncated
...
Add tests for duplicate file names, failed files, and changed manifests.

Add mocked-reader coverage for mixed flow cells at the `pod5-tools`
...
Expose playback planning only if synoptikon/mnemosyne needs simulated
run arrivals for QC/review workflows.

Keep write-capable POD5 subdivision out of R until read-only contracts
are stable.
...
Add a small derived-POD5 workflow for examples once the read-only
binding and provenance contracts are stable.
...
Expose `bam_verify(path)`, `bam_validate(path)`, and
`bam_check_eof(path)` through Rust-in-R.
...
Document which Porkchop capabilities are out of scope for floundeR
because they are preprocessing/trimming features rather than QC/review
evidence.
...
flowcell density, yield over time, quality distribution, barcode
balance, POD5 integrity, BAM alignment summaries, BAM
validation/index/sort evidence, Porkchop kit/library-preparation
...
or equivalent R API that produces HTML/PDF plus a report
...
Keep RMarkdown support only as a documented legacy/transitional path
until Grammateus reports cover the required technical-report workflow.
...
Add compatibility checks between floundeR versions and Grammateus
runtime versions.

Add CI separation: public floundeR checks without Grammateus,
credentialed private checks with Grammateus runtime/report rendering.
...
Commit and push changes after explicit prompt or as required by the
